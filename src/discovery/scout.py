...
def scout_jobs() -> list[JobListing]:
    ...
    for query in queries:
        ...
        try:
            ...
            for job in jobs:
                ...
                # Fast keyword pre-filter (runs before AI to reduce LLM calls)
                if title_keywords and not any(kw in title_lower for kw in title_keywords):
                    logger.debug(f"Skipping '{job.title}' — no title keyword match")
                    seen_ids.add(job.id)
                    continue

                if title_exclude and any(kw in title_lower for kw in title_exclude):
                    logger.debug(f"Skipping '{job.title}' — title excluded")
                    seen_ids.add(job.id)
                    continue
    ...
    for job in new_jobs:
        ...
        deduped.append(job)
    new_jobs = deduped
    logger.info(f"After dedup: {len(new_jobs)} unique candidates")
